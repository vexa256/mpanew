...
<div class="card-body shadow-lg pt-3 bg-light table-responsive">
    <div class="row">
        <div class="col-md-12">
            <form action="{{ route('ApprovedIndicatorReport') }}" method="GET">
                @csrf

                <input type="hidden" name="Entity" value="{{ $Entity }}">
                <div class="mb-3 col-md-12  py-5   my-5">
                    <label id="label" for="" class="px-5 my-5 required form-label">Select
                        Year | Only year with approved indicators are shown</label>
                    <select required name="ReportingYear" class="form-select  py-5   my-5 form-select-solid"
                        data-control="select2" data-placeholder="Select a option">
                        <option></option>
                        @isset($DataBaseData)

                            @foreach ($DataBaseData as $data)
                                <option value="{{ $data->ReportingYear }}">
                                    {{ $data->ReportingYear }}
